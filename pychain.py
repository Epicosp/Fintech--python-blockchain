...
sender = st.text_input("Senders address")

# @TODO:
# Add an input area where you can get a value for `receiver` from the user.
receiver = st.text_input("Receivers address")

# @TODO:
# Add an input area where you can get a value for `amount` from the user.
# give streamlit error if value is not digit or decimal
def is_float(value):
    try:
        float(value)
        return True
...
amount = st.text_input("Amount to be sent", 0)


if st.button("Add Block"):
    # quick check to make sure 'amount' is either int or float
    if amount.isdigit() or is_float(amount):
...
if st.button("Validate Chain"):
    if pychain.is_valid():
        st.write("The current pychain is valid.")
    else:
        st.write("The pychain is is not valid")
